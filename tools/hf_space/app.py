...
================================================================
Uses mp.solutions.holistic (mediapipe 0.9.3.0 / Python 3.9) — the same
model and settings as the local extract_holistic.py script, so output
...
def _get_landmarks(landmarks_obj):
    """Convert NormalizedLandmarkList → list of {x, y, z} dicts."""
...
def _extract_frames(video_path: str, start_sec: float = 0.0, end_sec: float = 0.0) -> list:
    """
    Process every frame through mp.solutions.holistic (smooth_landmarks=True),
    identical to local extract_holistic.py. Output is subsampled to ~150 frames
    after detection so Firestore stays under 1MB.
    """
    ...
    # before detection breaks holistic's temporal smoother)
