...
import '../services/dynamic_challenge_service.dart';
import '../services/analytics_service.dart';

class ChallengeProvider extends ChangeNotifier {
  final FirebaseFirestore _firestore = FirebaseFirestore.instance;
  final DynamicChallengeService _dynamicService = DynamicChallengeService();
  ...
  Future<void> loadChallenges(String userId, UserModel user) async {
    ...
    try {
      ...
      // ── Recalculate difficulty multiplier using yesterday's data ─────────
      final yesterday = now.subtract(const Duration(days: 1));
      final yesterdayKey =
          'daily_${yesterday.year}_${yesterday.month}_${yesterday.day}_type0';
      await _dynamicService.recalculateMultiplierIfNeeded(
        userId: userId,
        yesterdayPeriodKey: yesterdayKey,
      );
      ...
      // ── Append supplementary auto-generated challenges ────────────────────
      final supplementary = await _dynamicService.getSupplementaryTemplates(
        userId: userId,
        maxCount: 2,
      );
      for (final template in supplementary) {
        // Skip if a challenge with the same ID is already present
        if (_dailyChallenges.any((c) => c.id == template.id)) continue;
        final multiplier = await _dynamicService.getDifficultyMultiplier(userId);
        final scaled = _dynamicService.applyDifficulty(template, multiplier);
        final progress = await _getUserProgress(
            userId, user, scaled.trackingField, scaled.categoryId);
        _dailyChallenges.add(scaled.toChallenge().copyWith(
      ...
      debugPrint(
          '📦 Loaded challenges: ${_dailyChallenges.length} daily '
          '(${supplementary.length} auto), '
          '${_weeklyChallenges.length} weekly, '
          '${_specialChallenges.length} special');
    } catch (e) {
  ...
  Future<List<ChallengeModel>> _loadOrSelectChallenges({
    ...
    if (activeDoc.exists) {
      ...
      for (final id in challengeIds) {
        ...
        final scaled = _dynamicService.applyDifficulty(template, storedMultiplier);
        final progress = await _getUserProgress(
            userId, user, scaled.trackingField, scaled.categoryId);
        challenges.add(scaled.toChallenge().copyWith(
              currentValue: progress,
              isPersonalized: personalizedIds.contains(id),
              difficultyMultiplier: storedMultiplier,
            ));
      }
      return challenges;
    } else {
      // ── Select new challenges for this period ─────────────────────────────
      if (type == ChallengeType.daily) {
        return _selectDynamicDailyChallenges(
            userId, user, count, periodKey);
      } else {
        // Weekly and special: keep existing random selection
        final selected = await _selectRandomChallenges(type, count, periodKey);
        final challenges = <ChallengeModel>[];
        for (final template in selected) {
          final progress = await _getUserProgress(
              userId, user, template.trackingField, template.categoryId);
  ...
  /// Personalized + difficulty-adapted selection for a new daily period.
  Future<List<ChallengeModel>> _selectDynamicDailyChallenges(
    String userId,
    UserModel user,
    int count,
    String periodKey,
    ...
    try {
      // Fetch Firestore pool for daily type
      final snapshot = await _firestore
          .collection('challengePool')
          .where('type', isEqualTo: ChallengeType.daily.index)
      ...
      final result = await _dynamicService.selectDailyChallenges(
        userId: userId,
        pool: pool,
        count: count,
        periodKey: periodKey,
      ...
      final scaledTemplates = result.templates
          .map((t) => _dynamicService.applyDifficulty(t, multiplier))
          .toList();

      // Save selection to Firestore
      await _firestore.collection('activeChallenges').doc(periodKey).set({
        'challengeIds': scaledTemplates.map((t) => t.id).toList(),
        'personalizedIds': result.personalizedIds.toList(),
        'difficultyMultiplier': multiplier,
        'type': ChallengeType.daily.index,
        'createdAt': FieldValue.serverTimestamp(),
      });

      // Build ChallengeModel list
      final challenges = <ChallengeModel>[];
      for (final template in scaledTemplates) {
        final progress = await _getUserProgress(
            userId, user, template.trackingField, template.categoryId);
        challenges.add(template.toChallenge().copyWith(
              currentValue: progress,
              isPersonalized: result.personalizedIds.contains(template.id),
              difficultyMultiplier: multiplier,
            ));
      }
      return challenges;
    } catch (e) {
      debugPrint('⚠️ Dynamic daily selection failed, falling back: $e');
      final selected = await _selectRandomChallenges(
          ChallengeType.daily, count, periodKey);
      final challenges = <ChallengeModel>[];
      for (final template in selected) {
        final progress = await _getUserProgress(
            userId, user, template.trackingField, template.categoryId);
